...
\documentclass[conference,table]{IEEEtran} % Rudi: added [table] for row colours via xcolor
...
%noauthor_dalle2_nodate}), 
music~(MuseNet\footnote{\url{https://openai.com/research/musenet}}) and images~\cite{rombach_high-resolution_2022,ramesh_hierarchical_2022} (see \Cref{fig:introduction}),  among other outputs. %~\cite{noauthor_musenet_nodate} 
...
% Our contributions:
% - review of potential IP violations of training data for GAI models
% - a systematic review of technical solutions for protecting the IP of content available for training the large GAI models 
% - a taxonomy and classification of technical solutions 
% - a discussion on policy and practice revolving around IP issues in the context of GAI
...
%TODO: overall a bit too short; I think several works still need to be described better as to why they fall short; I added some comments, but please check it all together as well
...
% exclusion: works that do not evaluate on IP protection scenario are excluded, but those that describe similar or the same mechanisms in other scenarios such as privacy and harmful content removal are mentioned in section xy
\begin{figure}
    \centering
...
There are some special cases in the scope of a GAI process that describe special types of unauthorised usages:
        \paragraph{Unauthorised training} The AI provider uses the data for training the GAI model without authorisation, or a user uses the data for fine-tuning the models without authorisation. % todo: could expand
...
\end{figure*}
%TODO: is this a specific case of plagiarism mentioned above? or the other way around? Clarify & differentiate, either way if yes or if no
...
Some works argue that the memorisation in deep learning is an equivalent of overfitting~\cite{feldman_does_2020,feldman_what_2020,arpit_closer_2017}, %add paper with contrastive opinion? there is mem connected to overfitting and local mem
...
Generally, the works mentioned above~\cite{somepalli_diffusion_2022, somepalli_understanding_2023, carlini_extracting_2023, carlini_extracting_2021} are prone to underestimate the memorisation behaviour of generative models because of computing limitations that would (i) allow comparison with entire training sets of large diffusion models, and (ii) the limitations and inconsistencies of detection methods, i.e. not being able to find some memorised samples due to the missing capability of capturing examples that have semantic relevance, but are not detectable with current simple metrics. % also maybe mentioning the difference of more real-world replication and how it compares to induced memorization when we know the prompts from LAION  - (somepalli,carlini) (redact and also change place of this mention) current extraction scenarios consider cases where the original text prompts of training data are known and hence serve as a key to memory of the model 
% Among different setups considered in current research we should also differentiate induced memorization where we search among known original text prompts of training data and the scenario ...
...
%TODO: the message of the following paragraph is not clear to me; to some extend, it reads like it is an orthogonal topic, and not really causing an IPR issue; or is it? Maybe clarify that 
%(Tanja) -- yes it is the orthogonal problem which is (or can be) related to IPR; the text that follows ("It is important to note that membership inference...") highlights this relation. I think with a few of the recent edits this is fairly clear now, or?
...
Current works on representation disentanglement learning are mainly focused on unsupervised techniques incorporating encoders and modified loss functions into optimisation such as content-preservation loss and style-disentanglement loss~\cite{yang_disdiff_2023, wang_stylediffusion_2023, kwon_diffusion-based_2023, kazemi_style_2018}. %provide more citations, add for GANs
...
\input{taxonomy} %in progress: adjusting
...
Our proposed taxonomy is shown \Cref{fig:taxonomy}. We distinguish the methods along their characteristics of the \textit{protection mode}, the \textit{phase of application}, and their \textit{granulartiy}, as follows:
...
% The first category is \textit{Dataset sanitisation}, which requires direct access to the training set and modifying the underlying data before any training can take place -- with the goal of preventing IP violations further down the AI generation process. 
...
% Secondly, some methods use different forms of \textit{adversarial perturbations}. 
% These perturbations are applied to specific samples that need to be protected, and they cause the targeted behaviour in the downstream generation process. These methods are further described in \Cref{sec:mitigation-adversarial}.

% Thirdly, \textit{unlearning} and \textit{concept removal} as two distinct categories modify the model parameters; they rely on re-training or fine-tuning the model, hence they require access to the training process. These are described in \Cref{sec:mitigation-concept}.

% The umbrella term of \textit{watermarking} includes proactive detection techniques that modify the dataset, samples or concepts to ease later detection; these are described in \Cref{sec:mitigation-watermarking}.

% Finally, two groups of reactive detection methods do not modify the data collection, training or generation process but can rather serve as detection of potential violations (\textit{analytical attribution}) or an indicator that the additional preventive methods should be applied (\textit{testing memorisation}). We provide details about these methods in \Cref{sec:mitigation-detection}.
...
%TODO: Would be nice to have something similar for attacks as well? or are there too few methods that remove a protection method yet? some of them are inline in the text, I know, but maybe it would be good to group them together
...
% de-duplicating training data + image caption modification --> sanitising training data
...
% the trend of replication in diffusion models depends strongly on additional factors, related especially to their conditioning and to dataset complexity.
% e.g. "Caption specificity is closely tied to data duplication. Highly specific captions act like keys into the memory of the diffusion model that retrieve specific data points"
...
\paragraph{Image caption modification}\label{sec:mitigation-sanitising-caption} % check if more info or papers relevant
...
%     \caption{\textbf{Caption modification}: The most successful mitigation strategies for Stable Diffusion - \textit{multiple captions} for train time and \textit{random token/number addn.} for inference time. The top column represents train samples, the middle column is the memorised image and the bottom row is the generation with a mitigation strategy applied.~\cite{somepalli_understanding_2023}}
...
\hspace*{\fill}
  \subfloat[\label{fig:whitebox-workflow}Train time: multiple captions]{%
    ...
    \hfill
  \subfloat[\label{fig:blackbox-workflow}Inferene time: random token replacement / addition]{%
...
These strategies can be applied at train time to increase caption diversity among duplicates, but also at inference time which we discuss in the next paragraph.%or at inference time to disrupt the memorised connection between a caption and an image, 
...
Drawing the motivation from the well-researched realm of adversarial examples for discriminative models, %zhang_adversarial_2020}, %carlini&wagner2017,adry2018 
...
The adversarial example is then generated by adding subtle perturbation to the image to maximise the textural loss. As this approach targets the encoder, it can thus only protect against Latent diffusion models. % add how optimising textural loss implies targeting the encoder and the latent representation of the input, hence it can exclusively be applied to LDMs
...
While copyrighted images and artworks may be subject to such edits, unauthorised editing implies a broader range of misuse, such as privacy issues and spreading misinformation. % maybe some citation here %TODO: and also some explanation why this is a privacy issue
...
They further introduce a method for finding universal perturbations for unauthorised image editing that can be applied to protect a large number of images, without having to find optimal perturbations for each image. %TODO: explain how it differs from DUAW, which is also universal, right? DUAW also attacks the autoencoder (btw, be more precise in writing, do you mean the same with autoencoder and encoder? or the later is just the encoder part of the autoencoder? if so, then what is the difference in targeting the encoder or autoencoder?
...
Secondly, \textit{graphical distortions} compromise the perceptual quality of the generated image. %TODO: can we say something about what is better / more useful? what is the advantage of graphical? it is overall worse? 
...
%Some works propose LLM unlearning for classification tasks~\cite{pawelczyk_-context_2023}.
...
%In-context unlearning~\cite{pawelczyk_-context_2023} is a black-box unlearning method without direct access to the model parameters. 
%This method uses flipped labels and additional correctly labelled instances at inference time to eliminate the influence of a training point on the model output. --> evaluated for sentiment analysis
...
%TODO: I think it would be useful to discuss how detection can be used further downstream - i.e. what do you do with the information that something was detected as generated / IP infringing?
...
One such solution is the project \textit{Have I been trained?}\footnote{\url{https://haveibeentrained.com/}} %~\cite{noauthor_have_nodate} 
by the company \textit{Spawning}\footnote{\url{https://www.spawning.ai}}. %~\cite{noauthor_spawningai_nodate}. 
...
%TODO: I think it is not very clear what this section is about; maybe have some quick intro sentence that explains that
...
It promotes trustworthiness and transparency by helping fight disinformation as well as protecting the IPRs of digital content creators.%~\cite{noauthor_content_nodate}. 
...
% Longevity: ethical considerations besides adhering to policies
% call for promoting more improvements in protective technology and policies


%TODO: conclusions need to be much stronger

%TODO: I miss somewhere a discussion of the arms-race between protection and breaking those; drawing parallels to maybe adversarial examples and defences against those, and how those defences again can be broken, would be one interesting aspect
% In general, discussing whether protection is meaningful at all could be interesting
% Also, any recommendations for artists would be great - even if that is only to be careful on what they publish, where they publish it, how they publish it (e.g. low-res only), ...
