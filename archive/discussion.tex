...
% % e.f. violent content generation --> concept removal techniques
% % privacy -- adversarial noise
% Discuss relation difference btwn 
...
% LOFT -- optimise latent space; prevent image from changing, does not disrupt the final image, it just remains unchanged~\cite{zeng_loft_2024}

% DF-RAP -- make perturbations robust against compressiions perfromed by online social networks \cite{qu_df-rap_2024}

% Restricted black box -- they call it restricted black-box, but it's in fact a no-box (no queries); "a novel post-regularization module for enhancing the transferability of generated adversarial examples"\cite{dong_restricted_2023}

% Indirect deepfake watermarking --> IP watermarking
% Applied at sharing phase, optimised to persist in deepfake generation. 

% Deepfake disruption --> adversarial perturbations 
% Similarly to IP protection methods with adversarial perturbations, some deepfake distuprion methods aim to cause the low quality of the generated or nullify the functionality by injecting adversarial perturbations (degtrading visual quality) (white-box~\cite{disruptingdeepfakes ruiz}, black box~\cite{ruiz_practical_2023} or no box\cite{dong_restricted_2023}.
...
% CHALLENGES OF GROUPS OF METHODS
% todo: 
\subsection{Recommendations and Challenges}\label{sec:conclusion:challenges}
%TODO: state generally which of these methods require the GAI model/service operator to be active in defending? Which is everything in the beginning, resp. everything but cloaking?
